...
#' @param fourColor should a four color deck be applied. This means that each suit will have
...
#' @rdname suit
#' @export
suit_color <- function(suit = suit_list(), fourColor = TRUE) {
  suit <- match.arg(suit)
  if (fourColor)
...
#' @rdname suit
#' @export
suit_style <- function(suit, fourColor = TRUE) {
  requireNamespace("crayon")
  crayon::style(suit_unicode(suit), suit_color(suit, fourColor))
}

#' @rdname suit
#' @export
suit_html <- function(suit = suit_list(), size = 172, fourColor = TRUE, fill = TRUE) {
  suit <- match.arg(suit)
  unicode_html(
